...
export function ConversationList({
    ...
    return conversations
      .filter((conversation) => {
        ...
        if (
          ...
          !selectedPlatforms.has(conversation.platform)
        ) {
          return false
        }
        return true
  ...
  const filteredConversations = useMemo(() => {
    ...
    const items = convs.reduce<FilteredConversationItem[]>(
      (acc, conversation) => {
        const baseMatch = matchesSearch(conversation, normalizedSearchQuery)
        const summary = shouldRunMessageSearch
          ? resultSummaryMap[conversation.id]
          : undefined
        const textMatch = Boolean(summary)
        const matchesQuery =
          searchReadiness === "empty" ? true : baseMatch || textMatch
        ...
        const titleScore = parsedSearch
          ? scoreText(conversation.title ?? "", parsedSearch)
          : 0
        const rankScore = parsedSearch
          ? titleScore * 3 + (summary?.score ?? 0)
          : 0
        acc.push({
          conversation,
    ...
    if (isSearching) {
      items.sort(
        (a, b) =>
          ...
          timeOf(b.conversation) - timeOf(a.conversation)
      )
    } else {
